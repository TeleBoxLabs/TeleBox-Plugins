...
// Panel Settings Adapter
    panelAdapter: PanelSettingsAdapter = {
      ...
      getValues: async (): Promise<Record<string, unknown>> => {
        const db = await JSONFilePreset<PanguConfig>(path.join(createDirectoryInAssets("pangu"), "config.json"), { version: "1.0.0", chats: {}, whitelist: [], blacklist: [], globalMode: false } as any);
